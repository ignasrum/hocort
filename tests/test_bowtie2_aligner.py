...
bowtie2_output = f'{temp_dir.name}/output'
seq2 = f'{path}/test_data/sequences/sequences2.fastq'

def test_local_sam_1():
    options = ['--local']
    returncode, stdout, stderr = bt2.align_sam(idx, seq1, bowtie2_output, options=options)
    assert returncode == 0

def test_local_sam_2():
    options = ['--local']
    returncode, stdout, stderr = bt2.align_sam(idx, seq1, bowtie2_output, seq2=seq2, options=options)
    assert returncode == 0

def test_local_bam_1():
    options = ['--local']
    returncode, stdout, stderr = bt2.align_bam(idx, seq1, bowtie2_output, options=options)
    assert returncode[0] == 0
    assert returncode[1] == 0

def test_local_bam_2():
    options = ['--local']
    returncode, stdout, stderr = bt2.align_bam(idx, seq1, bowtie2_output, seq2=seq2, options=options)
    assert returncode[0] == 0
    assert returncode[1] == 0
