...
def test_pipeline_mfilter_true_1():
    returncode = BWA_MEM2().run(idx, seq1, out1, mfilter='t')
    assert returncode == 0

def test_pipeline_mfilter_false_1():
    returncode = BWA_MEM2().run(idx, seq1, out1, mfilter='f')
    assert returncode == 0

def test_pipeline_mfilter_true_2():
    returncode = BWA_MEM2().run(idx, seq1, out1, seq2=seq2, out2=out2, mfilter='t')
    assert returncode == 0

def test_pipeline_mfilter_false_2():
    returncode = BWA_MEM2().run(idx, seq1, out1, seq2=seq2, out2=out2, mfilter='f')
    assert returncode == 0
