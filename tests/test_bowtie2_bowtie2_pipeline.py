...
def test_pipeline_mfilter_true_1():
    returncode = Bowtie2Bowtie2().run(idx, seq1, out1, mfilter=True)
    assert returncode == 0

def test_pipeline_mfilter_false_1():
    returncode = Bowtie2Bowtie2().run(idx, seq1, out1, mfilter=False)
    assert returncode == 0

def test_pipeline_mfilter_true_2():
    returncode = Bowtie2Bowtie2().run(idx, seq1, out1, seq2=seq2, out2=out2, mfilter=True)
    assert returncode == 0

def test_pipeline_mfilter_false_2():
    returncode = Bowtie2Bowtie2().run(idx, seq1, out1, seq2=seq2, out2=out2, mfilter=False)
    assert returncode == 0
